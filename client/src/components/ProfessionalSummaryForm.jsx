...
const ProfessionalSummaryForm = ({data, onChange, setResumeData}) => {
    ...
    const generateSummary = async () => {
        try {
            ...
            if (response.data.enhancedContent) {
                onChange(response.data.enhancedContent); // update parent state
            } else {
  ...
  return (
    <div className='space-y-4'>
        ...
        <div className='mt-6'>
            <textarea rows={7} value={data || ""} onChange={(e)=> onChange(e.target.value)} className='w-full p-3 px-4 mt-2 border text-sm border-gray-300 rounded-lg focus:ring focus:ring-blue-500 focus:border-blue-500 outline-none transition-colors resize-none' name="" id="" placeholder='Write a compelling professional summary that highlights your key strengths and career objectives.' />
